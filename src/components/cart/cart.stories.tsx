...
import { LiquidUITheme } from '../liquid-ui/theme';
    ...
    design: [
      {
        name: 'Desktop & Mobile',
        type: 'figma',
        url: 'https://www.figma.com/file/hjdhnts9XKkReYO1oYkpqX/Liquid-UI?type=design&node-id=8055-50785&mode=dev',
...
export const Default: StoryObj<CartProps> = {
  render: (args) => `
      <button style="border: 1px solid; padding: 5px;" onclick="window.toggleCart()">
            Toggle Cart
          </button>
      <liquid-cart empty-cart-text="${args.emptyCartText}">
      </liquid-cart>
      <script>
        window.cartElement = document.querySelector('liquid-cart')

        window.cartElement.onClickContinueShopping = () => {
...
export const CustomThemeCart: StoryObj = {
  render: (args) => {
    ...
    return html`
      ...
      <liquid-ui theme="${theme}">
        <liquid-cart empty-cart-text="${args.emptyCartText}" theme="${theme}"> </liquid-cart>
      </liquid-ui>
      <script>
        window.cartElement = document.querySelector('liquid-cart');
  ...
  args: {
    ...
    theme: {
      ...
      cart: {
        root: {
          base: 'fixed right-0 bottom-0 h-screen sm:h-screen sm:p-4 z-50',
          sub: 'flex flex-col items-start h-screen sm:h-full w-screen sm:w-[480px] shadow-2xl border-none sm:rounded-[32px] bg-white min-h-[211px]',
        },
        footer: {
          wrapper: {
        ...
        body: {
          wrapper: {
            main: 'flex flex-col py-4 px-6 gap-6 overflow-auto max-h-[calc(100vh-133px)] sm:max-h-[calc(100vh-175px)]',
        ...
        singleRetailer: {
          wrapper: {
            ...
            sub: 'flex flex-row self-stretch pt-3 items-start gap-2 min-w-[312px] rounded-lg sm:rounded-2xl bg-[#F0F6FC]',
            height: 'min-h-screen sm:min-h-[calc(100vh-32px)]',
            sticky: 'sticky top-[100vh]',
            scroll:
              'overflow-y-auto h-[calc(100dvh-187px)] sm:h-[calc(100dvh-243px)] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]',
          ...
          text: {
            ...
            size: 'text-xs font-normal sm:text-sm text-[#333333]',
            fulfillment:
              'font-["Montserrat"] font-bold text-center text-[10px] sm:text-xs text-primary',
            quantity: 'w-8 sm:w-12 font-normal text-xs sm:text-sm text-center text-[#333]',
